...
export function NodePropertiesPanel(): React.ReactElement | null {
  ...
  const togglePropertiesPanel = useEditorStore(
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {selectedNode.type === "room" && (
            <>
              ...
              <Select
                ...
                onChange={(e) =>
                  updateNode(selectedNode.id, { shape: e.target.value })
                }
                options={[
          ...
          {selectedNode.type === "room_chain" && (
            <>
              ...
              <Select
                ...
                value={
                  (selectedNode.data.connectionStyle as string) ?? "linear"
                }
                onChange={(e) =>
                  updateNode(selectedNode.id, {
                    connectionStyle: e.target.value,
                  })
                }
                options={[
          ...
          {selectedNode.type === "spawn_point" && (
            <>
              ...
              <Input
                ...
                onChange={(e) =>
                  updateNode(selectedNode.id, {
                    spawnRadius: parseInt(e.target.value) || 2,
                  })
                }
              />
            </>
          ...
          {selectedNode.type === "merge" && (
            <>
              ...
              />
              <p className="text-xs text-text-muted">
                Determines how paths are combined when multiple branches
                converge.
              </p>
            </>
          )}
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-3">
                  <input
                    ...
                    max="100"
                    value={
                      ((selectedNode.data.dropChance as number) ?? 1) * 100
                    }
                    onChange={(e) =>
                      updateNode(selectedNode.id, {
                        dropChance: parseInt(e.target.value) / 100,
                      })
                    }
                    className="flex-1 accent-accent-primary"
                  />
            ...
            <>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  id="rewardOnComplete"
                  checked={
                    (selectedNode.data.rewardOnComplete as boolean) ?? true
                  }
                  onChange={(e) =>
                    updateNode(selectedNode.id, {
                      rewardOnComplete: e.target.checked,
                    })
                  }
                  className="rounded border-slate-600 bg-bg-tertiary text-accent-primary focus:ring-accent-primary"
                />
                <label
                  htmlFor="rewardOnComplete"
          ...
          {selectedNode.type === "output" && (
            <>
              <p className="text-xs text-text-muted">
                The exit point of the dungeon. Connects to the next level or
                ends the run.
              </p>
              <Select
                label="Exit Type"
