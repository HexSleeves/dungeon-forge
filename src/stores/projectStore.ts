...
const getDefaultPorts = (
  type: NodeType,
): { inputs: GraphNode["inputs"]; outputs: GraphNode["outputs"] } => {
  switch (type) {
    case "start":
      return {
        inputs: [],
        outputs: [
          { id: "out", type: "output", dataType: "room", label: "Out" },
        ],
      };
    case "output":
      return {
    ...
    case "prop":
      return {
        inputs: [
          { id: "room", type: "input", dataType: "room", label: "Room" },
        ],
        outputs: [],
      };
    default:
...
export const useProjectStore = create<ProjectState>()(
  immer((set, get) => ({
    ...
    removeGenerator: (id: string) => {
      ...
      set((state) => {
        if (state.project) {
          state.project.generators = state.project.generators.filter(
            (g) => g.id !== id,
          );
    ...
    updateNode: (id: string, data: Partial<GraphNode["data"]>) => {
      ...
      set((state) => {
        const generator = state.project?.generators.find(
          (g) => g.id === state.activeGeneratorId,
        );
    ...
    deleteNode: (id: string) => {
      ...
      set((state) => {
        ...
        if (generator) {
          generator.graph.nodes = generator.graph.nodes.filter(
            (n) => n.id !== id,
          );
    ...
    deleteConstraint: (id) => {
      ...
      set((state) => {
        ...
        if (generator) {
          generator.constraints = generator.constraints.filter(
            (c) => c.id !== id,
          );
        }
      });
